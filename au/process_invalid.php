<?php 
	...
	$property->bedrooms = strtoupper($_POST['bedrooms']);
	...
	$session->set_message(strtoupper($_POST['street_no']." ".$_POST['street_name']." ".$_POST['street_extension']." ".$_POST['suburb']));
	$session->log_end();

	if ($session->action == "INVALID"){
		$property->save();
		redirect_to($property->url);
		
	}  else if($_GET['action'] == 'delete'){
		$delete_invalid = Invalid::delete($_GET['id']);
		redirect_to("view.php");
	} 	
	redirect_to("invalid.php?id=0");
?>
