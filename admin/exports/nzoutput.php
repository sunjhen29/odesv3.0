<?php require_once("../../_includes/initialize.php");
	...
	fwrite($fh,"P/C1\tP/C2\t");
	for ($count = 3; $count <= 88; $count++){ fwrite($fh,"P".$count."\t");} 
	fwrite($fh,"P89\r\n");

	$nz_export = NewZealand::export_records($_GET['state'],$_GET['publication_name'],$_GET['publication_date']);
	$parent = false;
	$error = false;
	$record_id = 0;
	
	foreach($nz_export as $nz){
		...
		if($nz['pc'] == "P"){
			...
			fwrite($fh,$nz['pc']."\t".$sequence."\t".$nz['state']."\t".$nz['publication_name']."\t".$nz['publication_date']."\t".$nz['unit_no']."\t".$nz['street_no']."\t".$nz['street_no_suffix']."\t".$nz['street_name']."\t".$nz['street_extension']."\t");
