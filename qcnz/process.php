<?php 
	...
	if ($_GET['action'] == "modify"){
		...
		for($count=0; $count<=4; $count++){
			if ($agency_name[$count] != "" || $agent_contact[$count] != ""){
				...
				$addagent->publication_name = strtoupper($_POST['publication_name']);
	...
	redirect_to("default.php?&state=".$_POST['state']."&publication_name=".$_POST['publication_name']."&publication_date=".$_POST['publication_date']."&batch_id="); 
	} else if($_GET['action'] == 'delete'){
		$deleted_agent = Agent::delete_by_nzid($_GET['id']);
		$affected = NewZealand::delete($_GET['id']);
		$session->set_message("One record deleted!");
		redirect_to("default.php?&state=".$_GET['state']."&publication_name=".$_GET['publication_name']."&publication_date=".$_GET['publication_date']."&batch_id=");
	} 	
?>
