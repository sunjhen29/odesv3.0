...
class Australia{
	...
	public static function all_recordsperhour($proddate1,$proddate2){
		...
		$sql .= "SELECT 'ENTRY' as job, sum(if(entry_id!='',1,0)) as entry_records,entry_id,batch_id,publication_name,publication_date, data_entry_date,sum(entry_time) as total_time FROM au ";
		$sql .= "WHERE data_entry_date BETWEEN :data_entry_date1 AND :data_entry_date2 ";
		$sql .= "GROUP BY data_entry_date, entry_id ";
		$sql .= "UNION ";
		$sql .= "SELECT 'VERIFY' as job,sum(if(verify_id!='',1,0)) as entry_records,verify_id,batch_id,publication_name,publication_date,date_creation,sum(verify_time) as total_time FROM au ";
		$sql .= "WHERE date_creation BETWEEN :date_creation1 AND :date_creation2 ";
		$sql .= "GROUP BY date_creation,verify_id ";
		$sql .= "UNION ";
		$sql .= "SELECT 'ENTRY' as job, sum(if(entry_id!='',1,0)) as entry_records,entry_id,batch_id,publication_name,publication_date, data_entry_date,sum(entry_time) as total_time FROM invalid ";
		$sql .= "WHERE data_entry_date BETWEEN :data_entry_date1 AND :data_entry_date2 ";
		$sql .= "GROUP BY data_entry_date, entry_id ";
	...
	public static function export_records($state,$pubname,$pubdate){
		...
		$sql .= "suburb,city,property_type,listing_type,price_from,price_to,price_description,rental_period,DATE_FORMAT(auction_date,'%d/%m/%Y') as auction_date,replace(auction_time,' ','') as auction_time, ";//11-20
		...
		$sql .= "from au WHERE state=:state AND publication_name=:publication_name AND publication_date=:publication_date ";
		$sql .= "UNION ";
		$sql .= "SELECT au_id as sequence,'C' as pc,agency_name,agent_firstname,agent_surname,agent_contact,'','','','', ";
		$sql .= "'','','','','','','','','','', ";//20
		$sql .= "'','','','','','','','','','', ";//30
		$sql .= "'','','','','','','','','','', ";//40
		$sql .= "'','','','','','','','','','', ";//50
		...
		$sql .= "'','','','','','','','','','', ";//80
		$sql .= "'','','','','','' ";//86
		$sql .= "from agents_au WHERE state=:state AND publication_name=:publication_name AND publication_date=:publication_date ";
	...
	public static function view_records($optr,$proddate){
		...
		$sql  = "SELECT job_number as job, sum(if(entry_id!='',1,0)) as entry_records,entry_id,batch_id,publication_name,publication_date,data_entry_date,sum(entry_time) as total_time FROM au ";
		...
		$sql .= "UNION ";
		$sql .= "SELECT job_number2 as job,sum(if(verify_id!='',1,0)) as entry_records,verify_id,batch_id,publication_name,publication_date,date_creation,sum(verify_time) as total_time FROM au ";
	...
	public static function operator_stats($proddate){
		global $database;
		$production_date = DateTime::createFromFormat('d/m/Y',$proddate);
		$sql  = "SELECT RIGHT(job_number,1) as activity,LEFT(job_number,4) as job, DATE_FORMAT(data_entry_date,'%y') as year,DAYOFYEAR(data_entry_date) as juliandate,sum(if(entry_id!='',1,0)) as entry_records,entry_id,sum(entry_time) as total_time FROM au ";
		$sql .= "WHERE data_entry_date=:data_entry_date ";
		$sql .= "GROUP BY activity, entry_id, job ";
		$sql .= "UNION ";
		$sql .= "SELECT RIGHT(job_number2,1) as activity,LEFT(job_number2,4) as job,DATE_FORMAT(date_creation,'%y') as year,DAYOFYEAR(date_creation) as juliandate,sum(if(verify_id!='',1,0)) as entry_records,verify_id,sum(verify_time) as total_time FROM au ";
		$sql .= "WHERE date_creation=:date_creation ";
	...
	public static function viewbatches($state,$pubname,$pubdate){
		global $database;
		$publication_date = DateTime::createFromFormat('d/m/Y',$pubdate);
		$sql  = "SELECT GROUP_CONCAT(DISTINCT(entry_id),'|') as entry_id, GROUP_CONCAT(DISTINCT(verify_id),'|') as verify_id,batch_id,state,publication_name,publication_date,sum(if(entry_id!='',1,0)) as entry_records,sum(if(verify_id!='',1,0)) as verify_records,if(sum(if(entry_id!='',1,0))!=sum(if(verify_id!='',1,0)),'FOR VERIFY','FOR EXPORT') as status FROM au ";
		...
		$sql .= "UNION ";
		$sql .= "SELECT '' as entry_id,'' as verify_id,'' as batch_id,'' as state,'' as publication_name,'TOTAL :' as publication_date,sum(if(entry_id!='',1,0)) as entry_records,sum(if(verify_id!='',1,0)) as verify_records,if(sum(if(entry_id!='',1,0))!=sum(if(verify_id!='',1,0)),'FOR VERIFY','FOR EXPORT') as status FROM au ";
		$sql .= "WHERE state=:state AND publication_name=:publication_name AND publication_date=:publication_date ";
	...
	public static function records_summary($state,$pubname,$pubdate){
		global $database;
		$publication_date = DateTime::createFromFormat('d/m/Y',$pubdate);
		$sql = "SELECT count(state) as total, sum(if(sale_rent='SALE',1,0)) as sale, sum(if(sale_rent='RENT',1,0)) as rent FROM au ";
		$sql .= "WHERE state=:state AND publication_name=:publication_name AND publication_date=:publication_date ";
		$sql .= "GROUP BY state,publication_name,publication_date ";
		$database->query($sql);
		$database->bind(':state',$state);
	...
	public static function rowCount($batch_id,$state,$pubname,$pubdate){
		...
		$sql .= "FROM au WHERE batch_id = :batch_id AND state = :state AND publication_name = :publication_name ";
	...
	public static function find_next($batch_id,$state,$pubname,$pubdate){
		...
		$sql .= "street_no_suffix,street_name,street_extension,street_direction,suburb,city,entry_id,postcode, ";
		$sql .= "verify_id, property_type ";
		$sql .= "FROM au WHERE batch_id = :batch_id AND state = :state AND publication_name = :publication_name ";
		$sql .= "AND publication_date = :publication_date AND verify_id = :verify_id ORDER BY id ASC LIMIT 1";
		$database->query($sql);
	...
	Public static function find_property_details($id){
		global $database;
		$sql = "SELECT * from au WHERE id=:id";
		$database->query($sql);
		$database->bind(':id',$id);
		return $database->single();
	}
	...
	public static function auqc($batch_id,$state,$pubname,$pubdate){
		global $database;
		$publication_date = DateTime::createFromFormat('d/m/Y',$pubdate);
		$sql  = "SELECT id, batch_id, page_no,state, publication_name, publication_date,sale_rent,unit_no,street_no,";
		$sql .= "street_no_suffix,street_name,street_extension,street_direction,suburb,city,entry_id,postcode,";
		$sql .= "verify_id ";
		$sql .= "FROM au WHERE batch_id LIKE :batch_id AND state = :state AND publication_name = :publication_name ";
	...
	public function insert(){
		...
		$sql .= "(sequence_no,state,publication_name,publication_date,unit_no,street_no,street_no_suffix,street_name,street_extension,suburb,city,property_type,
				...
				data_entry_date,rp_property_id,rp_account_id,rp_personalized_id,page_no,batch_id,street_direction,sale_rent,entry_id,job_number,entry_time,postcode)";
		$sql .= " VALUES ";
		$sql .= "(:sequence_no,:state,:publication_name,:publication_date,:unit_no,:street_no,:street_no_suffix,:street_name,:street_extension,:suburb,:city,:property_type,
				:listing_type,:price_from,:price_to,:price_description,:rental_period,:auction_date,:auction_time,:auction_location,:water_frontage,:scenic_view,:air_conditioned,
				:heritage_other,:lift_installed,:access_security,:close_to_public,:vendor_will_trade,:permanent_water,:mains_electricity,:river_frontage,:coast_frontage,:canal_frontage,
		...
		$database->bind(':postcode',$this->postcode);
		$database->execute();
	...
	public function verify(){
		...
		$sql .= "SET sequence_no = :sequence_no,state = :state,publication_name = :publication_name,publication_date = :publication_date,unit_no = :unit_no,street_no = :street_no,
				...
				rp_personalized_id = :rp_personalized_id,page_no = :page_no,batch_id = :batch_id,street_direction = :street_direction,sale_rent = :sale_rent,verify_id = :verify_id,
				date_creation=:date_creation,job_number2=:job_number2,verify_time=:verify_time,postcode=:postcode,time_stamp2=:time_stamp2 ";
		$sql .= "WHERE id = :id";
		$database->query($sql);
		...
		$database->bind(':postcode',$this->postcode);
		$database->bind(':time_stamp2',date('Y-m-d H:i:s'));
		$database->execute();		
	}
	public function update(){
		...
		$sql .= "SET sequence_no = :sequence_no,state = :state,publication_name = :publication_name,publication_date = :publication_date,unit_no = :unit_no,street_no = :street_no,
				...
				construction_type = :construction_type,materials_in_roof = :materials_in_roof,type_of_scenic = :type_of_scenic,ad_size = :ad_size,ad_photo_type = :ad_photo_type,ad_photo_count = :ad_photo_count,
				ad_section = :ad_section,ad_exclusive = :ad_exclusive,additional_property = :additional_property,rp_property_id = :rp_property_id,rp_account_id = :rp_account_id,
				rp_personalized_id = :rp_personalized_id,page_no = :page_no,batch_id = :batch_id,street_direction = :street_direction,sale_rent = :sale_rent,postcode = :postcode ";
		$sql .= "WHERE id = :id";
		$database->query($sql);
		$database->bind(':id',$this->id);
		$database->bind(':sequence_no',$this->sequence_no);
	...
	public function get_columns(){
		global $database;
		$sql = "SHOW COLUMNS FROM au";
		$database->query($sql);
		return $database->resultset();
	}
	public static function debug($id=0){
		global $database;
		$sql = "SELECT * FROM au";
		$database->query($sql);
		return $database->resultset();
