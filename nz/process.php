<?php 
	...
	if ($session->action == "ENTRY" || $session->action == "VERIFY" || $session->action == "MODIFY"){
		$property = new NewZealand();
		$property->id=$_POST['id'];
		$property->sequence_no = "";
		$property->state = strtoupper($session->state);
		$property->publication_name = strtoupper($session->publication_name);
		$publication_date = DateTime::createFromFormat('d/m/Y',$session->publication_date);
		...
		$property->city = strtoupper($_POST['city']);
		...
		$property->batch_id = strtoupper($session->batch_id);
		...
		$property->verify_id = $session->operator_id;
		$property->job_number = $session->job_number."E";
		$property->job_number2 = $session->job_number."V";
		$property->entry_time = time() - $session->start;
		$property->verify_time = time() - $session->start;
		...
		if($session->action == "ENTRY"){
			...
			for($count=0; $count<=4; $count++){
				if ($agency_name[$count] != "" || $agent_contact[$count] != ""){
					...
					$addagent->agent_contact = $agent_contact[$count];
					$addagent->publication_name = $session->publication_name;
					$addagent->publication_date = $session->publication_date;
					$addagent->state = $session->state;
					$addagent->addAgent();
				}
			}
			$session->set_message(strtoupper($_POST['street_no']." ".$_POST['street_name']." ".$_POST['street_extension']." ".$_POST['suburb']." ".$_POST['city']));
			$session->log_end();
			redirect_to("address.php?id=0");
		} elseif($session->action == "VERIFY"){
			$property->verify();
			for($count=0; $count<=4; $count++){
				if ($agency_name[$count] != "" || $agent_contact[$count] != ""){
					$addagent = new Agent();
					$addagent->id = !empty($agent_id[$count]) ? $agent_id[$count] : 0;
					$addagent->nz_id = $property->id;
					$addagent->agency_name = strtoupper($agency_name[$count]);
					$addagent->agent_firstname = strtoupper($agent_firstname[$count]);
					$addagent->agent_surname = strtoupper($agent_surname[$count]);
					...
					if($agent_id[$count]){
						$deleted = Agent::delete($agent_id[$count]);
					}
				}
			}
			$session->log_end();
			redirect_to("verify.php");
		} elseif($session->action == "MODIFY"){
			$property->update();
			for($count=0; $count<=4; $count++){
				if ($agency_name[$count] != "" || $agent_contact[$count] != ""){
					$addagent = new Agent();
		...
		$deleted_agent = Agent::delete_by_nzid($_GET['id']);
		$affected = NewZealand::delete($_GET['id']);
		redirect_to("view.php");
	} 	
